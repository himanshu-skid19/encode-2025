...
import { SALES_PROMPT } from "../lib/constants";
...
// Constants
const DEEPGRAM_SOCKET_URL = process.env.NEXT_PUBLIC_DEEPGRAM_SOCKET_URL as string;
const PING_INTERVAL = 8000; // 8s
const CUSTOMER_ID = 1001; // Hardcoded customer ID for the demo
...
export const WebSocketProvider = ({ children }: WebSocketProviderProps) => {
  const { token } = useAuth();
  console.log("Initial auth token status:", !!token);
  // State
  ...
  const [customerPrompt, setCustomerPrompt] = useState<string>(SALES_PROMPT);
  ...
  const [configSettings, setConfigSettings] = useState({
    ...
    agent: {
      ...
      think: {
        ...
        model: model.split("+")[1],
        instructions: customerPrompt,
      },
      speak: { model: voice },
    },
  });

  const fetchCustomerPrompt = useCallback(async () => {
    try {
      const customerData = await customerService.getById(CUSTOMER_ID);
      const customizedPrompt = PromptService.generateCustomPrompt(customerData);
      setCustomerPrompt(customizedPrompt);
      ...
      setCustomerPrompt(SALES_PROMPT);
    }
  }, []);
  ...
  useEffect(() => {
    if (token) {
      ...
      console.log("No token available for API key fetch");
    }
  }, [token]);
